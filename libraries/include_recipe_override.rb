module RunlistModifiers
  module IncludeRecipe
    ...
    def modifier_include_recipe(*recipe_names)
      ...
      recipe_names.collect  do |recipe_name|
        begin
          unless recipe_name.start_with?('runlist_modifiers')
            if is_a?(Chef::RunContext)
              if !_fetch_recipes(:allowed_recipes).empty? && !_fetch_recipes(:allowed_recipes).include?(recipe_name.to_s)
                ...
                raise Chef::Exceptions::RestrictedRecipe, recipe_name
              end
            end
          ...
          msg = if e.recipe_name == recipe_name
                  'Restricted recipe encountered:'
                else
                  "Restricted recipe dependency found. (#{recipe_name} depends on #{e.recipe_name})."
                end
          Chef::Log.warn msg << " #{recipe_name} -> Not Loaded"
          raise e unless is_a?(Chef::RunContext)
        end
      end.compact
    end
    ...
    def _fetch_recipes(key)
      ...
      unless @_mod_recipe_cache[key.to_sym]
        @_mod_recipe_cache[key.to_sym] = [node[key.to_sym]].flatten.compact.map do |item|
